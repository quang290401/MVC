                ...
                <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                    <div class="text-center">
                        <a class="btn btn-outline-dark mt-auto"
                           href="<c:url value="/detail?idProduct=${o.idProduct }&idUser=${USERMODEL.idUser}"/>">View
                            options</a>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $('#btnAddCart').click(function (e) {
        var gioHangExists = false;
        var sanPhamExists = false;
        e.preventDefault();
        var data = {};
        ...
        $.each(formData, function (i, v) {
            console.log(v)
            data["" + v.name + ""] = v.value;
            if (v.name === 'gioHang' && v.value.trim() !== '') {
                gioHangExists = true;
            }

            // Kiểm tra xem giá trị sanPham đã tồn tại hay không
            if (v.name === 'sanPham' && v.value.trim() !== '') {
                sanPhamExists = true;
            }
        });
        if ( sanPhamExists && gioHangExists) {
            addNew(data);
    ...
    function isProductInCart(data) {
        var gioHangId = data.gioHang;
        var sanPhamId = data.sanPham;
        ...
        var existingProduct = cart.find(item => item.gioHang === gioHangId && item.sanPham === sanPhamId);

        if (existingProduct) {
            // Sản phẩm đã có trong giỏ hàng
            return confirm('Sản phẩm đã có trong giỏ hàng. Bạn có muốn cập nhật số lượng không?');
        } else {
